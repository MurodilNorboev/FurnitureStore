...
const DatailRoomComponent: React.FC<Tnames> = ({ name }) => {
  ...
  const spliced = _id?.split("}").join("");
  const dispatch = useDispatch<AppDispatch>();
  const cartsData = useSelector((state: RootState) => state.carts.items);
  ...
  const fetchData = async () => {
    try {
      ...
      const filter2 = data.filter((item: any) => item._id === spliced);
      setData(filter2);
    } catch (error: any) {
  ...
  const combinendData = [mock, ...data].flat();
      ...
      <div className="wrapper">
        <div className="imgwrap1">
          {combinendData.map((val: any, ind: any) => (
            <div
              key={`${val.categories}-${val.id} || ${val._id} && ${ind}`}
              className="imgswrap"
            >
              <Tab
                active={activeTab === 1}
                ...
                className="imgs"
              >
                <img src={val.images || val.image} alt={`Image for Tab 1`} />
              </Tab>
              <Tab
                active={activeTab === 2}
        ...
        <RightCon className="RightCon">
          ...
          <div className="right_cart">
            <div className="bottom_Con bottom2">
              <div className="cost">
                {data?.map((val, ind) => (
                  <div key={val.id || ind} className="h3">
                    ${val.cost}
                  </div>
                ))}
            ...
            <div className="bottom_Con">
              {combinendData.map((val, ind) => {
                return (
                  <div className="cons_btn" key={`${val.id} || ${val._id} || ${ind}`}>
                    <button onClick={() => handleAddCart(val._id)}>
                      Add to cart
                    </button>
                  </div>
                );
...
//       <h2>Filtered Data:</h2>
//       {combinendData.length > 0 ? (
//         combinendData.map((item, index) => (
...
//                   alt=""
//                 />
//                  <button onClick={() => handleAddCart(item._id)}>
//                   Add to cart
//                 </button>
//               </>
//             ) : (
...
//     cartsData.map((item) => (
//       <div key={item._id}>
//         <img src={item.image} alt="" style={{ width: "100px", borderRadius: "5px" }} />
//         <p>{item.types}</p>
//         <p>{item.cost}</p>
//       </div>
//     ))
