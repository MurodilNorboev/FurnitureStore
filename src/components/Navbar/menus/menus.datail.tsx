...
import '../../main.all_categorie/i.css'
...
export default function Ssofar(Props: Tname) {
  ...
  const filteredData = label === "new in"
  ? data.filter(item => item.new === "new") // new: 'new' bo'lgan barcha elementlar
  : data.filter(item => item.label === label); // label bo'yicha filtrlash
